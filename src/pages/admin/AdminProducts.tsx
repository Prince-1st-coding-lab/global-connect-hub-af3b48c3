...
/** Raw image values (storage paths) are kept for saving; previews are signed. */
const useRawProduct = (id?: string) =>
  useQuery({
    enabled: !!id,
    queryKey: ["admin-product-raw", id],
    queryFn: async () => {
            ...
            <div key={p.id} className="flex gap-4 rounded-xl border border-border p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {p.category_id ? categoryName.get(p.category_id) ?? "Uncategorised" : "Uncategorised"} ·{" "}
                  {p.stock} in stock
                </p>
